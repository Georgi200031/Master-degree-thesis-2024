...
class StockGraphicGui:
    ...
    def __init__(self, root):
        ...
        # Log Area
        self.log_area = tk.Text(self.root, height=13, width=90, state="disabled", bg="#e0e0e0")
    ...
    def create_grid_search_inputs(self):
        ...
        # Hidden Layers
        self.hidden_layers_label = tk.Label(self.grid_search_frame, text="Hidden Size:", bg="#f0f0f0")
        ...
        # Iteration Layers
        self.hidden_layers_from_label = tk.Label(self.grid_search_frame, text="Settings for grid search:", bg="#f0f0f0")
        self.hidden_layers_from_label.grid(row=5, column=1, padx=5, pady=5, sticky='w')

        self.iterations_label = tk.Label(self.grid_search_frame, text="iterations Size:", bg="#f0f0f0")
        self.iterations_label.grid(row=6, column=0, padx=5, pady=5, sticky='w')
        self.iterations_to_entry = tk.Entry(self.grid_search_frame, width=10)
        self.iterations_to_entry.grid(row=6, column=1, padx=5, pady=5, sticky='ew')
        self.iterations_to_entry.insert(0,"800")
    ...
    def get_grid_search_parameters(self):
        ...
        iterations = int(self.iterations_to_entry.get())
        print(iterations)
        return hidden_layers, learning_rates, regularization_strengths, momentums, iterations
    ...
    def load_data(self):
        ...
        self.log(f"Loading data for {crypto}...")
        train_data = TrainingData(self.stock, start_date, end_date, split_percentage, self.algorithm_settings, self.log)
        ...
        if mode == "grid_search":
            for entry in entries:
                ...
                if not text:  # Check if the text is empty or contains only alphanumeric characters
                    ...
                    self.__init__()
                if text_float < 0:
                    messagebox.showerror("Error", "Text boxes cannot be negative.")
                    self.__init__()
    ...
    def on_grid_search_click(self):
        ...
        hidden_layers, learning_rates, regularization_strengths, momentums, iterations = self.get_grid_search_parameters()
        self.algorithm_settings.set_grid_search_parameters(iterations, hidden_layers, learning_rates, regularization_strengths, momentums)
